...
一份精心整理的开源清单，收录 **HTML 演示文稿生成 Skill**——可被 AI 编程 Agent（Claude Code / Codex / Hermes / OpenClaw）加载的打包技能，以及配套的 HTML 模板库。一句话、一份大纲、一篇 Markdown，即可生成零依赖、单文件 HTML 演示文稿。

本仓库收录的全部是真实可访问的 GitHub 仓库。星标数通过 GitHub API 实时获取，并作为排序的唯一依据。简介与亮点来自项目自身 README，并在表述上做了一致性梳理。
...
**亮点**：极舒服的「风格发现」流程；活跃的 fork 生态（见下方 `frontend-slides-editable`）；Anthropic 官方将其作为 Skill 能力的展示案例。
...
发现链接失效、星标过期、或我们漏掉的项目？欢迎 [提 Issue](https://github.com/ToseaAI/issues/new)。
